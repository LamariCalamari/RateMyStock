...
# - Robust chunked peer loader (retries, threads off)
# - 12m momentum via 5Y history for DISPLAYED tickers
# - Fundamentals / Technicals / Macro with friendly text
# - Submit-based portfolio editor (Apply / Normalize)
# - Charts with titles & captions
# - Clean, centered landing page + inline SVG logo
# - Deterministic peer sampling; safer NASDAQ-100 list
# - Fixed pandas truth-value bug in macro; many edge-case guards
...
def fetch_prices_chunked_with_fallback(
    ...
    chunk=40, retries=3, sleep_between=0.25,
    singles_pause=0.18, hard_limit=350
):
    ...
    for i in range(0, len(tickers), chunk):
        ...
        try:
            ...
            pass
        time.sleep(sleep_between + random.uniform(0, 0.12))
        ...
        for _ in range(retries):
            ...
            for t in missing:
                try:
                    ...
                    new_missing.append(t)
                time.sleep(singles_pause + random.uniform(0, 0.22))
            missing = new_missing
...
def fetch_vix_series(period="6mo", interval="1d") -> pd.Series:
    # Returns Series or empty Series on failure
    try:
        ...
        pass
    # fallback attempt: longer period
    try:
...
def fetch_fundamentals_simple(tickers):
    ...
    for raw in tickers:
        ...
        try:
            tkr = yf.Ticker(t)
            # try fast_info, then .info
            try:
                fi = getattr(tkr, "fast_info", None)
                info = dict(fi) if fi else {}
            except Exception:
                info = {}
            if not info:
                info = tkr.info or {}
...
NASDAQ100_STATIC = {
    # Trimmed/representative, maintain as needed
    "AAPL","MSFT","NVDA","AMZN","META","GOOGL","AVGO","TSLA","COST","ADBE","NFLX","AMD",
    "PEP","CSCO","INTC","QCOM","TXN","AMAT","ISRG","BKNG","INTU","PDD","SBUX","MU",
    "MDLZ","GILD","ADI","REGN","VRTX","LRCX","ABNB","PANW","KLAC","SNPS","CRWD","MRVL",
    "ASML","LIN","ORLY","CDNS","NXPI","PCAR","WDAY","FTNT","ROP","PAYX","ODFL","DXCM",
    "MELI","CEG","EXC","ROST","EA","CTSH","ALGN","FAST","VRSN","KHC","AEP","MSCI","IDXX"
}
...
def list_nasdaq100():
    # yfinance doesn't provide a NASDAQ-100 helper; avoid using tickers_nasdaq()
    return set(NASDAQ100_STATIC)
...
def build_universe(user_tickers, mode, sample_n=150, custom_raw=""):
    ...
    peers_pool = list(peers_all.difference(set(user)))
    random.seed(42)  # deterministic sampling
    random.shuffle(peers_pool)
    peers = peers_pool[:max(1, sample_n)]
...
def macro_from_vix(vix_series: pd.Series):
    # Must avoid truth-value checks on pandas objects (the crash you saw)
    ...
    e20_series = ema(vix_series,20) if len(vix_series)>=20 else pd.Series([vix_last], index=[vix_series.index[-1]])
    vix_ema20 = float(e20_series.iloc[-1])
    denom = vix_ema20 if (not np.isnan(vix_ema20) and vix_ema20 != 0.0) else 1e-9
    rel_gap  = (vix_last - vix_ema20) / denom
    ...
    return macro, vix_last, vix_ema20, rel_gap
...
def app_stock():
    ...
    with st.status("Crunching the numbers…", expanded=True) as status:
        ...
        prices, ok = fetch_prices_chunked_with_fallback(
            ...
            chunk=40, retries=3, sleep_between=0.25, singles_pause=0.18
        )
        if not ok:
    ...
    for t in show_idx:
        try:
            px5 = yf.Ticker(t).history(period="5y", interval="1d")
            if isinstance(px5, pd.DataFrame) and not px5.empty and "Close" in px5:
                px5 = px5["Close"].dropna()
                if len(px5)>253:
    ...
    st.caption("Scores are peer-relative z-scores blended by your weights; Macro is uniform.")
    ...
    for t in show_idx:
        ...
        with st.expander(f"{t} — {reco} (Score: {sc:.1f})", expanded=True):
            ...
            # Charts
            try:
                px2df = yf.Ticker(t).history(period="2y", interval="1d")
                px2 = px2df["Close"].dropna() if isinstance(px2df, pd.DataFrame) and "Close" in px2df else pd.Series(dtype=float)
                if px2.size > 0:
...
def holdings_editor_form(currency_symbol, total_value):
    ...
    with st.form("holdings_form", clear_on_submit=False):
        # segmented_control fallback for older Streamlit
        try:
            ...
            sync_mode = st.radio("Sync mode", ["Percent → Amount", "Amount → Percent"], index=0)
...
def app_portfolio():
    ...
    meta_sec = pd.Series({t: fetch_sector(t) or "Unknown" for t in tickers})
    ...
    cols_held = [c for c in tickers if c in prices.columns]
    px_held = prices[cols_held].dropna(how="all") if cols_held else pd.DataFrame()
    if px_held.shape[1] >= 2:
        ret = px_held.pct_change().dropna(how="all")
    ...
    if not px_held.empty:
        ...
        st.info("No overlapping price history among holdings for charts.")
