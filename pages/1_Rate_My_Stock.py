...
from app_utils import (
    ...
    fetch_prices_chunked_with_fallback, fetch_macro_pack, score_universe_panel,
    fundamentals_interpretation, build_universe, SCORING_CONFIG, score_label, FACTOR_LABELS,
    # NEW:
...
with st.status("Crunching the numbers…", expanded=True) as status:
    ...
    status.update(label="Loading macro regime…")
    msg.info("Step 3/5: combining macro signals (VIX, USD, rates, credit, gold).")
    macro_pack = fetch_macro_pack(period="6mo", interval="1d")
    prog.progress(65)
    pct.markdown("**Progress: 65%**")

    status.update(label="Scoring fundamentals + technicals…")
    msg.info("Step 4/5: running the upgraded scoring engine.")
    ...
    status.update(label="Finalizing outputs…")
    msg.info("Step 5/5: computing ratings, confidence, and recommendations.")
...
if out.empty:
    st.error("Unable to compute scores for this peer universe.")
    st.stop()
...
for t in show_idx:
    ...
    with st.expander(f"{t} — {reco} (Score: {sc:.1f})", expanded=True):
        ...
        # Top drivers
        factor_map = FACTOR_LABELS.copy()
        zrow = {}
        if t in fdf.index:
            zrow.update({k: fdf.loc[t, k] for k in fdf.columns if k in factor_map})
        if t in tech.index:
            for k in factor_map:
